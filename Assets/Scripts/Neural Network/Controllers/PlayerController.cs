...
namespace Neural_Network.Controllers
{
    public class PlayerController : ActionController
    {
        ...
        private void NeuronsMove()
        {
            ...
            var playerPosition = (transform.position - _opponent.transform.position) * _verySmallValue;
            _tempValues[4] = playerPosition.x;
            _tempValues[5] = playerPosition.y;

            var enemyPosition = (_opponent.transform.position - transform.position) * _verySmallValue;
            _tempValues[6] = enemyPosition.x;
            _tempValues[7] = enemyPosition.y;
            ...
            var leftDistance = (playerPosition.x - _wallLeft.position.x) * _verySmallValue;
            var rightDistance = (_wallRight.position.x - playerPosition.x)  * _verySmallValue;
            var botDistance = (playerPosition.y - _wallBot.position.y) * _verySmallValue;
            ...
            _tempValues[16] = opponentCanAttack;

            var scale = transform.localScale.x;
            var oponentScale = _opponent.transform.localScale.x;

            _tempValues[17] = scale;
            _tempValues[18] = oponentScale;
            ...
            if (left >= 0.0f) ActionMoveLeft();
            if (right >= 0.0f) ActionMoveRight();
            
            if (jump >= 0.0f) ActionJump();
            if (attack >= 0.0f) ActionAttack();
        }
    }
}
